...
def test_financial_conglomerates_portfolio_legal_person_type_maturity_schema_structure():
    """Deve ter os nomes das colunas e tipos corretos definidos."""
    schema = FinancialConglomeratePortfolioLegalPersonTypeMaturitySchema()
...
def test_financial_conglomerates_portfolio_legal_person_type_maturity_transform_integration(
    mocker,
    mock_financial_conglomerates_portfolio_legal_person_type_maturity_csv_data,
    transformer_factory,
    mock_dataframe_from_csv,
):
    """Should transform Financial Conglomerates Portfolio Legal Person Type Maturity data correctly."""

    # Create mock DataFrame using the shared helper
    schema = FinancialConglomeratePortfolioLegalPersonTypeMaturitySchema()
    mock_df = mock_dataframe_from_csv(
        mock_financial_conglomerates_portfolio_legal_person_type_maturity_csv_data, schema
    )
