...
def test_financial_conglomerates_portfolio_individuals_type_maturity_schema_structure():
    """Deve ter os nomes das colunas e tipos corretos definidos."""
    schema = FinancialConglomeratePortfolioIndividualsTypeMaturitySchema()
...
def test_financial_conglomerates_portfolio_individuals_type_maturity_transform_integration(
    mocker,
    mock_financial_conglomerates_portfolio_individuals_type_maturity_csv_data,
    transformer_factory,
    mock_dataframe_from_csv,
):
    """Should transform Financial Conglomerates Portfolio Individuals Type Maturity data correctly."""

    # Create mock DataFrame using the shared helper
    schema = FinancialConglomeratePortfolioIndividualsTypeMaturitySchema()
    mock_df = mock_dataframe_from_csv(mock_financial_conglomerates_portfolio_individuals_type_maturity_csv_data, schema)
