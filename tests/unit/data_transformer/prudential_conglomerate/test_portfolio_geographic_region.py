...
def test_prudential_conglomerates_portfolio_geographic_region_schema_structure():
    """Test that the Prudential Conglomerates Portfolio Geographic Region
    schema has correct column names and transformation types.
    """

    schema = PrudentialConglomeratePortfolioGeographicRegionSchema()
...
def test_prudential_conglomerates_portfolio_geographic_region_transform_integration(
    mocker,
    mock_prudential_conglomerate_portfolio_geographic_region_csv_data,
    transformer_factory,
    mock_dataframe_from_csv,
):
    """Should transform Prudential Conglomerates Portfolio Geographic Region data correctly."""

    # Create mock DataFrame using the shared helper
    schema = PrudentialConglomeratePortfolioGeographicRegionSchema()
    mock_df = mock_dataframe_from_csv(mock_prudential_conglomerate_portfolio_geographic_region_csv_data, schema)
