...
def test_prudential_conglomerates_portfolio_legal_person_business_size_schema_structure():
    """Test that the Prudential Conglomerates Portfolio Legal Person Business Size schema has
    correct column names and transformation types.
    """

    schema = PrudentialConglomeratePortfolioLegalPersonBusinessSizeSchema()
    ...
    expected_structure = {
        ...
        'indisponivel': TransformationType.NUMERIC,
...
def test_prudential_conglomerates_portfolio_legal_person_business_size_transform_integration(
    mocker,
    mock_prudential_conglomerate_portfolio_legal_person_business_size_csv_data,
    transformer_factory,
    mock_dataframe_from_csv,
):
    """Should transform Prudential Conglomerates Portfolio Legal Person Business Size data correctly."""

    # Create mock DataFrame using the shared helper
    schema = PrudentialConglomeratePortfolioLegalPersonBusinessSizeSchema()
    mock_df = mock_dataframe_from_csv(
        mock_prudential_conglomerate_portfolio_legal_person_business_size_csv_data, schema
    )
