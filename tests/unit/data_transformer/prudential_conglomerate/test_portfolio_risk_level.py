"""Tests for Prudential Conglomerates Portfolio Risk Level data transformation schema."""
...
def test_prudential_conglomerates_portfolio_risk_level_schema_structure():
    """Test that the Prudential Conglomerates Portfolio Risk Level schema has
    correct column names and transformation types.
    """

    schema = PrudentialConglomeratePortfolioRiskLevelSchema()
    ...
    expected_structure = {
        ...
        'a': TransformationType.NUMERIC,
        'b': TransformationType.NUMERIC,
        'c': TransformationType.NUMERIC,
        'd': TransformationType.NUMERIC,
        'e': TransformationType.NUMERIC,
        'f': TransformationType.NUMERIC,
        'g': TransformationType.NUMERIC,
        'h': TransformationType.NUMERIC,
...
def test_prudential_conglomerates_portfolio_risk_level_transform_integration(
    mocker, mock_prudential_conglomerate_portfolio_risk_level_csv_data, transformer_factory, mock_dataframe_from_csv
):
    """Should transform Prudential Conglomerates Portfolio Risk Level data correctly."""

    # Create mock DataFrame using the shared helper
    schema = PrudentialConglomeratePortfolioRiskLevelSchema()
    mock_df = mock_dataframe_from_csv(mock_prudential_conglomerate_portfolio_risk_level_csv_data, schema)
