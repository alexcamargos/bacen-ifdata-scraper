"""Tests for Financial Conglomerate Capital Information data transformation schema."""

from pathlib import Path

import pandas as pd

from bacen_ifdata.data_transformer.controller import (
    TransformationType,
...
def test_financial_conglomerates_capital_information_schema_structure():
    """Test that the Financial Conglomerate Capital Information schema has
    correct column names and transformation types.
    """

    schema = FinancialConglomerateCapitalInformationSchema()
...
def test_financial_conglomerates_capital_information_transform_integration(
    mocker, mock_financial_conglomerates_capital_information_csv_data, transformer_factory, mock_dataframe_from_csv
):
    """Should transform Financial Conglomerate Capital Information data correctly."""

    # Create mock DataFrame using the shared helper
    schema = FinancialConglomerateCapitalInformationSchema()
    mock_df = mock_dataframe_from_csv(mock_financial_conglomerates_capital_information_csv_data, schema)
