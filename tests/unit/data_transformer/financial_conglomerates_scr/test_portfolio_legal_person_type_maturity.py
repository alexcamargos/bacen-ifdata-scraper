"""Tests for Financial Conglomerates Portfolio Legal Person Type Maturity data transformation schema."""
...
def test_financial_conglomerates_scr_portfolio_legal_person_type_maturity_schema_structure():
    """Test that the Financial Conglomerates SCR Portfolio Legal Person Type Maturity schema
    has correct column names and transformation types.
    """

    schema = FinancialConglomerateSCRPortfolioLegalPersonTypeMaturitySchema()
    ...
    categories = [
        ...
        'operacoes_com_recebiveis',
        'comercio_exterior',
        'outros_creditos',
        'financiamento_infraestrutura',
...
def test_financial_conglomerates_scr_portfolio_legal_person_type_maturity_transform_integration(
    mocker,
    mock_financial_conglomerates_scr_portfolio_legal_person_type_maturity_csv_data,
    transformer_factory,
    mock_dataframe_from_csv,
):
    """Should transform Financial Conglomerates SCR Portfolio Legal Person Type Maturity data correctly."""

    # Create mock DataFrame using the shared helper
    schema = FinancialConglomerateSCRPortfolioLegalPersonTypeMaturitySchema()
    mock_df = mock_dataframe_from_csv(
        mock_financial_conglomerates_scr_portfolio_legal_person_type_maturity_csv_data, schema
    )
    mocker.patch(
        'bacen_ifdata.data_transformer.controller.load_csv_data',
