"""Tests for the Financial Conglomerates SCR Portfolio Geographic Region data transformation schema."""
...
def test_financial_conglomerates_scr_portfolio_geographic_region_schema_structure():
    """Test that the Financial Conglomerates SCR Portfolio Geographic Region
    schema has correct column names and transformation types.
    """

    schema = FinancialConglomerateSCRPortfolioGeographicRegionSchema()
...
def test_financial_conglomerates_scr_transform_integration(
    mocker,
    mock_financial_conglomerates_scr_portfolio_geographic_region_csv_data,
    transformer_factory,
    mock_dataframe_from_csv,
):
    """Should transform Financial Conglomerates SCR Portfolio data correctly."""

    # Create mock DataFrame using the shared helper
    schema = FinancialConglomerateSCRPortfolioGeographicRegionSchema()
    mock_df = mock_dataframe_from_csv(mock_financial_conglomerates_scr_portfolio_geographic_region_csv_data, schema)
